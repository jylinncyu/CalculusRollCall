...
<html lang="en">
<head>
    <title><?php echo $_SESSION["department_name"]?></title>
    <meta charset="utf-8">
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    if (isset($_POST['optradio'])){
      ...
      for($i=0; $i<count($ID); $i++){
        $sql = "UPDATE `studentlist` SET `Hide` = '1' WHERE `studentlist`.`ID` ='{$ID[$i]}'";
    ...
    if ((isset($_SESSION['activity_id']))&(isset($_SESSION['year']))){
      ...
      if (!is_null($row['excel_name'])){
        echo "<a href=\"xlsx\\{$row['excel_name']}\" class=\"btn btn-info\" role=\"button\">下載學生參加活動清單Excel檔</a>";
      ...
      $sql = "SELECT * FROM `studentlist` WHERE `Year` = '$year' and `activity` = '$activity_name'";
...
<?php  
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    echo "<div class=\"text-center\">";
    echo "<a href=\"output_attendence_list.php?year={$year}&activity_name={$activity_name}\" class=\"btn btn-info\" role=\"button\">產生學生參加活動清單Excel檔</a>";
    echo "<a href=\"output_attendence_list2.php?year={$year}&activity_name={$activity_name}\" class=\"btn btn-info\" role=\"button\">產生學生參加活動清單Excel檔(詳細)</a>";
    echo "</div>";
